...
Claude Code Studio 把多个 Claude Code CLI 会话变成一个协作团队。一个会话负责策略和派活，其他会话分头执行——所有人通过共享的 MCP server 沟通（可选 Redis pub/sub 做实时推送）。
...
- **跨模型审查** — OpenAI 官方 [codex-plugin-cc](https://github.com/openai/codex-plugin-cc) 插件直接在 studio 里工作，让 Codex (GPT-5.4) 审查 Claude 写的代码——两家竞争对手的 AI 在同一个工作区里协作
...
- Redis —— **可选**，仅用于实时 pub/sub 推送（Docker: `docker run -d -p 6379:6379 redis:7-alpine`）。默认的 SQLite 后端无需任何额外依赖。
- Claude Code CLI
...
| `STUDIO_BACKEND` | `sqlite` | 存储后端：`sqlite`（零依赖）或 `redis`（实时 pub/sub） |
...
> 启动脚本的 Redis 还绑在 `0.0.0.0` 且无鉴权，只建议在可信网络里这么用。

## 跨模型协作

Claude Code Studio 天然支持跨模型工作流。通过 OpenAI 官方的 [codex-plugin-cc](https://github.com/openai/codex-plugin-cc) 插件，你可以让 Codex (GPT-5.4) 审查 Claude agent 写的代码——就在 studio 内部完成。
...
│       │                                           │
│       ▼                                           │
│   Codex (GPT-5.4)   审查代码 (/codex:review)       │
│       │                                           │
│       ▼                                           │
│   Claude (Opus)     验证审查结论并修复              │
│                                                   │
│   两家竞争对手的 AI，在同一个工作区里搭班干活。       │
└──────────────────────────────────────────────────┘
```
